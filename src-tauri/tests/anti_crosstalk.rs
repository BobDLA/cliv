...
fn claude_session_isolation() {
    ...
    let result_a = cliv_lib::extract::claude::extract_claude_reply_from(
        home,
        Some("session-aaa".to_string()),
    );
    assert_eq!(result_a.unwrap(), "Reply from Claude session AAA");

    let result_b = cliv_lib::extract::claude::extract_claude_reply_from(
        home,
    ...
    let result_a2 = cliv_lib::extract::claude::extract_claude_reply_from(
        home,
        Some("session-aaa".to_string()),
    );
    assert!(
...
fn gemini_session_isolation() {
    ...
    let result_1 = cliv_lib::extract::gemini::extract_gemini_reply_from(
        home,
        Some("pid-1000".to_string()),
    );
    assert_eq!(result_1.unwrap(), "Reply from Gemini session 1000");

    let result_2 = cliv_lib::extract::gemini::extract_gemini_reply_from(
        home,
        Some("pid-2000".to_string()),
    );
    assert_eq!(result_2.unwrap(), "Reply from Gemini session 2000");

    // Cross-check: pid-1000 must not read pid-2000's data
    let result_1b = cliv_lib::extract::gemini::extract_gemini_reply_from(
        home,
        Some("pid-1000".to_string()),
    );
    assert!(
...
fn codex_thread_isolation() {
    ...
    write_cache(home, "thread-xxx", "Reply from Codex thread XXX");
    write_cache(home, "thread-yyy", "Reply from Codex thread YYY");

    let result_x = cliv_lib::extract::codex::extract_codex_reply_from(
        home,
        Some("thread-xxx".to_string()),
    );
    assert_eq!(result_x.unwrap(), "Reply from Codex thread XXX");

    let result_y = cliv_lib::extract::codex::extract_codex_reply_from(
        home,
        Some("thread-yyy".to_string()),
    );
...
fn no_session_returns_error_not_random_data() {
    ...
    assert!(claude_err.is_err(), "Claude should error without session ID");
    assert!(
    ...
    assert!(gemini_err.is_err(), "Gemini should error without session ID");
...
fn missing_cache_returns_error_with_path() {
    ...
    assert!(
        ...
        "Error should mention the session ID, got: {}", msg
    );
    assert!(
        msg.contains("reply_cache"),
        "Error should mention the expected path, got: {}", msg
    );

    let err = cliv_lib::extract::gemini::extract_gemini_reply_from(
        home,
        Some("ghost-pid".to_string()),
    );
    assert!(err.is_err());
    let msg = err.unwrap_err();
